...
document.addEventListener("DOMContentLoaded", async () => {
	// Initialize save status indicator first
	initSaveStatusIndicator();

	// Add timeout to hide loading overlay if auth listener never fires
	const authTimeoutId = setTimeout(() => {
		...
		if (loadingOverlay && loadingOverlay.style.display !== "none") {
			...
			// Redirect to login if auth check takes too long
	...
	try {
		const unsubscribe = initAuthListener(async (user) => {
			...
			try {
				window.showBudgetWarningBanner?.();
			} catch (e) {
				/* noop */
			}
		});
		...
		// Redirect to login on critical Firebase error
		setTimeout(() => {
			window.location.href = "login.html";
		}, 1000);
	}
