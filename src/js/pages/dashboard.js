...
async function showEditBaseModal(state) {
	...
	window.showModal({
		...
		onSave: async () => {
			...
			try {
				if (window.initTransactions) await window.initTransactions();
			} catch (e) {}
			try {
...
async function showAddCategoryModal() {
	...
	window.showModal({
		...
		onSave: async () => {
			...
			try {
				...
				if (base > 0) {
					const totalAssigned = s.categories
						.filter((c) => c.type !== "income")
	...
	setTimeout(() => {
		...
		function toggle() {
			limitInput.style.display = typeSel.value === "income" ? "none" : "block";
		}
...
function renderDashboard(state) {
	...
	document.getElementById("budget-base").textContent =
		`Base Budget: $${Number(state.meta.baseBudget || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
			...
			.forEach((btn) => {
				btn.addEventListener("click", async () => {
					...
					if (action === "delete") {
						if (
							confirm(
								"Delete this category? This will not remove transactions.",
							)
						) {
						...
						window.showModal({
							...
							onSave: async () => {
								...
								const newType =
									document.getElementById("edit-cat-type").value || "expense";
								let newLimit;
								if (newType === "expense") {
									...
									const st = await getState();
									const baseBudget = Number(st.meta.baseBudget || 0);
									if (baseBudget > 0 && newLimit > baseBudget) {
										alert("Category limit cannot exceed the Base Budget.");
										return false;
									}
								} else {
						...
						setTimeout(() => {
							...
							function toggle() {
								limitEl.style.display =
									typeEl.value === "income" ? "none" : "block";
							}
	...
	expenseList.querySelectorAll("button[data-action]").forEach((btn) => {
		btn.addEventListener("click", async () => {
			...
			if (action === "delete") {
				if (
					confirm("Delete this category? This will not remove transactions.")
				) {
					await removeCategory(id);
					const s = await getState();
					renderDashboard(s);
	...
	await new Promise((res) => {
		window.showModal({
			...
			onSave: async () => {
				...
				if (Math.abs(sum - remaining) > 0.01) {
					alert(
						`Allocated sum $${sum.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} does not equal remaining $${remaining.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}.`,
					);
					return false;
				}
		...
		setTimeout(() => {
			...
			evenBtn?.addEventListener("click", () => {
				...
				document
					.querySelectorAll(".alloc-input")
					.forEach((i) => (i.value = per));
			});
		}, 10);
	});
}
